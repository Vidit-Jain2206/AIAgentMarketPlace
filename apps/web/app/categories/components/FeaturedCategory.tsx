...
const FeaturedCategory = ({
  ...
  return (
    ...
    >
      ...
      >
        <BsFire className="text-2xl text-fuchsia-500 animate-pulse" />
        <span className="text-fuchsia-400 font-space-mono text-sm tracking-widest uppercase">
          Trending This Week
        </span>
      </motion.div>
      ...
      <motion.div
        ...
        className="relative bg-black/60 backdrop-blur-xl border border-fuchsia-500/30 p-6 sm:p-8 overflow-hidden group cursor-pointer hover:border-fuchsia-400/60 transition-all duration-500"
        style={{
          clipPath:
        ...
        <div className="absolute top-4 right-4 flex gap-1">
          {[...Array(3)].map((_, i) => (
            <motion.div
              ...
              className="w-2 h-2 bg-fuchsia-400 rounded-full blur-sm"
            />
          ))}
        </div>
          ...
          <div className="flex-1">
            <div className="flex items-center gap-4 mb-4">
              <div className="p-3 bg-fuchsia-400/10 border border-fuchsia-400/30">
                <FiTrendingUp className="text-3xl text-fuchsia-400" />
              </div>
              <div>
                <h2 className="text-2xl sm:text-3xl font-orbitron font-bold text-white group-hover:text-fuchsia-400 transition">
            ...
            <div className="flex flex-wrap gap-4 mb-6">
              <span className="px-4 py-2 bg-fuchsia-500/10 border border-fuchsia-400/30 text-fuchsia-400 font-space-mono text-sm">
                {agentCount} AGENTS
              </span>
            ...
            <motion.button
              whileHover={{ x: 5 }}
              className="flex items-center gap-2 text-fuchsia-400 hover:text-white font-orbitron text-sm transition group/btn"
            >
              EXPLORE CATEGORY
          ...
          <div className="flex gap-4 flex-wrap lg:flex-nowrap">
            {topAgents.slice(0, 3).map((agent, index) => (
              <motion.div
                ...
                className="bg-black/80 border border-gray-800 p-4 min-w-[140px] hover:border-cyan-400/50 transition group/agent"
              >
                <div className="w-10 h-10 bg-gray-900 border border-gray-700 rounded-lg flex items-center justify-center mb-3 group-hover/agent:border-cyan-400/50 transition">
                  <BsRobot className="text-gray-600 group-hover/agent:text-cyan-400 transition" />
                </div>
                <p className="text-white font-rajdhani font-semibold text-sm truncate group-hover/agent:text-cyan-400 transition">
                  {agent.agentName}
                </p>
